...
module.exports = async (tweet, bg = "default") => {
  ...
  let style_form = fs.readFileSync(
    path.join(__dirname, "form", "style.scss"),
    "utf-8"
  );
  ...
  const verified = fs.readFileSync(
    path.join(__dirname, "form", "verified.html"),
    "utf-8"
  );
  const play = fs.readFileSync(
    path.join(__dirname, "form", "play.html"),
    "utf-8"
  );
  ...
  const text_tagged = text
    ...
    .replace(
      /(?<=\s)#([^\s]+)/g,
      '<a href="https://twitter.com/hashtag/$1?src=hashtag_click" target="_blank">#$1</a>'
    )
    .replace(
      /(?<=\s)@([^\s]+)/g,
  ...
  let main = fs.readFileSync(
    path.join(__dirname, "form", "main.html"),
    "utf-8"
  );
  ...
  main = author.verified
    ? main.replace("%VERIFIED%", verified)
    : main.replace("%VERIFIED%", "");
  ...
  //
  let includes = [];
  let include_form = fs.readFileSync(
    path.join(__dirname, "form", "include.html"),
    "utf-8"
  );
  ...
  if (tweet.includes.media && tweet.includes.media[0].type === "photo") {
    ...
    let include_img = fs.readFileSync(
      path.join(__dirname, "form", "include", "media", `image_${img_cnt}.html`),
      "utf-8"
    );
    ...
    for (let i = 1; i <= img_cnt; i++) {
      include_img = include_img.replace(
        `%IMAGE_URL_${i}%`,
        tweet.includes.media[i - 1].url
      );
    }
  ...
  if (tweet.includes.media && tweet.includes.media[0].type === "video") {
    ...
    let include_video = fs.readFileSync(
      path.join(__dirname, "form", "include", "media", "video.html"),
      "utf-8"
    );

    // Filling data in the form
    include_video = include_video.replace("%LINK%", link);
    include_video = include_video.replace(
      `%PREVIEW_IMAGE_URL%`,
      tweet.includes.media[0].preview_image_url
    );
  ...
  if (tweet.includes.polls) {
    ...
    let include_poll = fs.readFileSync(
      path.join(
        __dirname,
        "form",
        "include",
        "poll",
        `poll_${poll_status}.html`
      ),
      "utf-8"
    );
    const include_poll_option = fs.readFileSync(
      path.join(
        __dirname,
        "form",
        "include",
        "poll",
        `poll_${poll_status}_option.html`
      ),
      "utf-8"
    );
    ...
    const votes_all = tweet.includes.polls[0].options.reduce((a, c) => {
      return a + c.votes;
    }, 0);
    const votes_max = tweet.includes.polls[0].options.reduce((a, c) => {
      return c.votes > a ? c.votes : a;
    }, 0);
    ...
    for (let i = 1; i <= option_cnt; i++) {
      ...
      const this_option_percent = `${
        Math.round((this_option.votes / votes_all) * 1000) / 10
      }%`;

      let include_poll_this_option = include_poll_option;
      include_poll_this_option = include_poll_this_option.replace(
        /\%LINK\%/g,
        link
      );
      include_poll_this_option = include_poll_this_option.replace(
        `%IS_TOP_OPTION%`,
        this_option.votes === votes_max ? "top" : ""
      );
      include_poll_this_option = include_poll_this_option.replace(
        `%POLL_OPTION_LABEL%`,
        this_option.label
      ...
      include_poll_this_option = include_poll_this_option.replace(
        `'%POLL_OPTION_BG_WIDTH%'`,
        this_option_percent
      );

      include_poll_options.push(include_poll_this_option);
    }
    include_poll = include_poll.replace(
      "%POLL_OPTIONS%",
      include_poll_options.join("")
    );
    include_poll = include_poll.replace(
      "%VOTES_ALL%",
      locale.poll.unit.replace("%NUM%", votes_all)
    );
